...
Thread::Yield (int status)
{
    ...
    nextThread = scheduler->FindNextToRun(status);
    DEBUG('t', "Next thread \"%s\"\n", (nextThread==NULL)?"No thread left":nextThread->getName());
    if (nextThread != NULL) {
        // the FindNextToRun function help us put the loser back to the ready list
        // thus we should just run the next thread
...
Thread::Sleep ()
{
    ...
    status = BLOCKED;
    while ((nextThread = scheduler->FindNextToRun(Complete)) == NULL)
	interrupt->Idle();	// no one to run, wait for an interrupt

    DEBUG('t', "Thread \"%s\" complete, Next thread \"%s\"\n",currentThread->getName(), nextThread->getName());
