...
// from the virtual address
    ...
    if (writing){
         ...
         * Lab6:mup extension
         * copy on writing
         * check if the cur page table belongs to cur thread
         * if not,copy the page table to the cur thread's page table
         ...
         */
         ...
         if(flag){
             ...
             if(currentThread->getPID()!=oas->getASID()){
                 // allocate a new addr space for cur thread
                 TranslationEntry* nPT = new TranslationEntry[oas->getNumPages()];
                 for(int i=0;i<oas->getNumPages;i++){
                     nPT[i].virtualPage = i;
                     nPT[i].physicalPage = pageMap->Find();
                     //copy the data
                     for(int j=0;j<PageSize;j++){
                         machine->mainMemory[nPT[i].physicalPage*PageSize+j]=
                                 machine->mainMemory[oas->getPT()[i].physicalPage*PageSize+j];
                     }
                     nPT[i].valid = TRUE;
                     nPT[i].use = FALSE;
                     nPT[i].dirty = FALSE;
                 ...
                 // update the as id for next check
                 currentThread->space->setASID(currentThread->getPID());
             }
         }
    }
    /**
